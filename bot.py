...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if uid not in pending_users:
        await update.message.reply_text(f"⚠️ No pending request for UID: `{uid}`", parse_mode="Markdown")
        return
    ...
    await update.message.reply_text(f"✅ Approved and invite sent for UID: `{uid}`", parse_mode="Markdown")
...
async def reject(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(
        chat_id=telegram_user_id,
        text="❌ Your UID could not be verified. Please contact support.",
    )
